...
router.get('/move/:name', async (req, res) => {
    try {
        ...
        const simplifiedData = {
            ...
            effect: moveData.effect_entries.find((entry) => entry.language.name === 'en')?.effect || '',
            shortEffect: moveData.effect_entries.find((entry) => entry.language.name === 'en')?.short_effect || ''
        };
        res.json(simplifiedData);
    }
    catch (error) {
